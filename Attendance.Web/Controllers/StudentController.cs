...
namespace Attendance.Web.Controllers
{
    public class StudentController : Controller
    {
        ...
        public StudentController(IAttendanceManager attmgr)
        {
            _attmgr = attmgr;
        }
        // GET: Student
        ...
        {
            var model = _attmgr.GetStudent(id);
            ViewBag.levels = new SelectList(_attmgr.GetLevels(), "LevelId", "LevelName");
        ...
        public ActionResult Edit(StudentModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                _attmgr.Update(model.StudentId, model);
                return RedirectToAction("Index");
            }
            return View(model);
        }
        ...
        public ActionResult Delete(int id, string FirstName)
        {
            ...
            {
                try { 
                _attmgr.RemoveStudent(id);
                    return Json(new { status = true, message = $" Student has been successfully deleted!", JsonRequestBehavior.AllowGet });
                }
                catch (Exception ex)
                {
                ...
                if (idss.Count() > 0)
                {
                    foreach (var strid in idss)
                    {
                        if (!string.IsNullOrEmpty(strid))
                        {
                            ...
                            _attmgr.RemoveStudent(intid);
                        }
                    }
                        return Json(new { status = true, message = " All selected student(s) has been successfully deleted!", JsonRequestBehavior.AllowGet });
